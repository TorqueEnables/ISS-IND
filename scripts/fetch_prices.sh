...
# -*- coding: utf-8 -*-
"""
ISS-IND robust EOD fetcher (concurrent cold-seed, time-boxed, archives-only)

Outputs (atomic writes under data/):
  - data/deliverables_latest.csv, data/deliverables_hist.csv   (best-effort MTO)
  - data/bulk_deals_latest.csv, data/block_deals_latest.csv    (best-effort APIs)
  - data/prices/bhav_hist.csv                                   (cold-seeded if empty)
  - data/highlow_52w.csv                                        (derived)
  - data/tech_latest.csv                                        (latest-day indicators)
  - data/fetch_status.json                                      (diagnostics; URLs tried)

Environment overrides (optional):
  SEED_MIN_DAYS    -> default 90 (distinct trading days to collect)
  SEED_MAX_SECONDS -> default 720 (12 minutes ceiling for seeding step)
  SEED_WORKERS     -> default 8 (concurrent fetchers)
"""

from __future__ import annotations
import os, sys, io, json, time, zipfile, threading
...
SEED_MIN_DAYS = int(os.environ.get("SEED_MIN_DAYS", "90"))
SEED_MAX_SECONDS = int(os.environ.get("SEED_MAX_SECONDS", "720"))  # 12 minutes
SEED_WORKERS = max(1, int(os.environ.get("SEED_WORKERS", "8")))
...
# ----------------- HTTP session with tight retries -----------------
def get_session() -> requests.Session:
    ...
    for url in [
        ...
        try: s.get(url, timeout=10)
        except Exception: pass
    return s

# ----------------- Deliverables (best-effort) -----------------
...
def try_mto_for_date(s: requests.Session, d: datetime) -> Optional[pd.DataFrame]:
    ...
    for u in urls:
        try:
            r = s.get(u, timeout=12, headers={"Referer": "https://www.nseindia.com/market-data"}, allow_redirects=True)
...
def fetch_corporates(s: requests.Session, kind: str, status_urls: Dict[str, Any]) -> Optional[pd.DataFrame]:
    ...
    for url in eps[kind]:
        ...
        try:
            r = s.get(url, timeout=12, headers={"Accept":"application/json, text/plain, */*", "Referer": referer}, allow_redirects=True)
...
def load_bhav_latest_local_or_remote() -> Optional[pd.DataFrame]:
    ...
    try:
        r = requests.get(GH_RAW_BHAV_URL, timeout=10, headers={"User-Agent":"curl/8.0"})
...
# ----------------- Concurrent cold-seed -----------------
def fetch_day_from_archives(s: requests.Session, d: datetime) -> Tuple[str, Optional[pd.DataFrame], List[str]]:
    """Try sec_bhavdata_full (CSV), then zipped cm..bhav.csv.zip (archives only), http fallback.
       Returns (date_str, df or None, urls_tried)."""
    urls_tried: List[str] = []
    # 1) sec_bhavdata_full
    ddmmyyyy = d.strftime("%d%m%Y")
    csv_urls = [
    ...
    for url in csv_urls:
        ...
        try:
            r = s.get(url, timeout=12, headers={"Referer":"https://www.nseindia.com/market-data"}, allow_redirects=True)
            if r.status_code == 200 and r.text:
                ...
                if not out.empty:
                    return ddmmyyyy, out, urls_tried
        except Exception:
            pass
    # 2) zipped cm..bhav.csv.zip (archives only)
    ...
    for url in zip_urls:
        urls_tried.append(url)
        try:
            r = s.get(url, timeout=18, headers={"Referer":"https://www.nseindia.com/market-data"}, allow_redirects=True)
            ...
            pass
    return ddmmyyyy, None, urls_tried

def concurrent_cold_seed(s: requests.Session, status_urls: Dict[str, Any], min_days:int, max_seconds:int, workers:int) -> int:
    """Try to collect >= min_days distinct trading days within max_seconds, concurrently."""
    ...
    start = time.time()
    candidates = previous_business_days(240)  # 240 business days back (~1 year)
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futures = {ex.submit(fetch_day_from_archives, s, d): d for d in candidates}
        for fut in as_completed(futures):
            ddmmyyyy, df, urls = fut.result()
            with lock:
                tried_urls.extend(urls)
            # Time budget check
            ...
            if df is not None and not df.empty:
                ...
                with lock:
                    if day not in seen_dates:
                        ...
                        seen_dates.add(day)
                        # small, bounded log
                        # print(f"Seeded {day} rows={len(df)}")  # noisy in CI; skip
                        if len(seen_dates) >= min_days:
                            break

    status_urls.setdefault("bhav_tried", []).extend(tried_urls[:2000])  # cap to keep JSON reasonable
...
def main():
    ...
    try:
        ...
        status["steps"]["bulk"] = {"ok": bool(bdf is not None and not bdf.empty), "rows": 0 if bdf is None else int(len(bdf))}
    ...
    try:
        ...
        status["steps"]["block"] = {"ok": bool(kdf is not None and not kdf.empty), "rows": 0 if kdf is None else int(len(kdf))}
        if kdf is not None and not kdf.empty:
            atomic_write_df(kdf, FILES["block_latest"])
    except Exception as e:
        status["steps"]["block"] = {"ok": False, "error": str(e)}

    # Bhav: append bhav_latest if exists; else concurrent seed with time budget
    seeded_rows = 0
    try:
        ...
        if bl is not None and not bl.empty:
            ...
            atomic_write_df(allb, FILES["bhav_hist"])
        # If still empty, seed concurrently within max_seconds
        seeded_rows = concurrent_cold_seed(s, status["debug"], SEED_MIN_DAYS, SEED_MAX_SECONDS, SEED_WORKERS)
    except Exception as e:
